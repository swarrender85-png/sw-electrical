...
(function () {
  ...
  // No key configured, or still a placeholder: go straight to the fallback
  // rather than requesting a script that cannot work.
  if (!c.googleMapsApiKey || c.googleMapsApiKey.indexOf('INSERT') === 0) {
    fallback();
    return;
  }

  // Google calls this global on any auth failure: invalid key, referrer not
  // allowed, billing not enabled, API not activated. It fires AFTER Google has
  // already painted its grey "Sorry! Something went wrong" panel, so clear it.
  ...
  // Belt and braces: if the script neither loads nor errors (blocked by an
  // extension, dead network), nothing above ever fires.
